...
def main(
    ...
    if interactive:
        ...
        while True:
            ...
            completion_tokens = generate(model, prompt_tokens, max_new_tokens, -1, temperature)
            completion = decode(completion_tokens[0])
            print(completion)
    else:
        with open(input_file) as f:
            prompts = [line.strip() for line in f.readlines()]
        assert len(prompts) <= args.max_batch_size
        prompt_tokens = [encode(prompt) for prompt in prompts]
        completion_tokens = generate(model, prompt_tokens, max_new_tokens, -1, temperature)
        completions = [decode(tokens) for tokens in completion_tokens]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input-file", type=str, default="data/prompt.txt")
